...
test('installing preserves the user\'s own hook and formatting', async () => {
  ...
  try {
    ...
    assert.ok(after.includes('echo benim-kendi-hookum'), 'the user own hook was deleted');
    assert.ok(/\n {4}"theme"/.test(after), `indentation changed:\n${after}`);

    // Ours landed on Stop as a *separate* matcher, not by editing theirs.
    const stop = child(child(root, 'hooks'), 'Stop')!;
    assert.equal(stop.items?.length, 2, 'our own matcher should have been added separately');
...
test('uninstalling removes only ours and restores the original meaning', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      'after removal the file must mean the same thing',
    );
...
test('installing twice changes nothing the second time', async () => {
  ...
  try {
    ...
    await installHooks({ ...ARGS, settingsPath: s.path });
    assert.equal(readFileSync(s.path, 'utf8'), once, 'a second install must not change the file');
  } finally {
    s.cleanup();
  }
});
...
test('a missing settings file is created, not crashed on', async () => {
  ...
  try {
    ...
    assert.ok(child(root, 'hooks'), 'hooks were not written');
  } finally {
    ...
    for (const m of hooks.members ?? []) {
      for (const matcher of m.value.items ?? []) {
        for (const h of child(matcher, 'hooks')?.items ?? []) {
          assert.equal(child(h, '_vt')?.value, true, `${m.key}: no marker`);
...
test('high fidelity adds the tool events and nothing else changes', async () => {
  ...
  try {
    ...
    assert.ok(!events.includes('MessageDisplay'), 'MessageDisplay must never be wired up');
  } finally {
    s.cleanup();
  }
});
